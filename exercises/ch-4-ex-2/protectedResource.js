...
app.get('/words', getAccessToken, requireAccessToken, function (req, res) {
	...
	if (req.access_token.scope.includes('read')) {
		...
		res.set('WWW-Authenticate', 'Bearer realm=localhost:9002, error="insufficent_scope", scope="read"');
		res.status(403).end();
	}
});
...
app.post('/words', getAccessToken, requireAccessToken, function (req, res) {
	...
	if (req.access_token.scope.includes('write')) {
		...
		res.set('WWW-Authenticate', 'Bearer realm=localhost:9002, error="insufficent_scope", scope="write"');
		res.status(403).end();
	}
...
app.delete('/words', getAccessToken, requireAccessToken, function (req, res) {
	...
	if (req.access_token.scope.includes('delete')) {
		...
		res.set('WWW-Authenticate', 'Bearer realm=localhost:9002, error="insufficent_scope", scope="delete"');
		res.status(403).end();
	}
